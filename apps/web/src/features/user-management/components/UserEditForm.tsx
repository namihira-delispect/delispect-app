...
import type { UpdateUserFormState, UserDetail } from "../types";
...
export function UserEditForm({ user }: UserEditFormProps) {
  ...
  const boundUpdateUserAction = updateUserAction.bind(null, user.id);

  const [state, formAction, isPending] = useActionState(
    boundUpdateUserAction,
    initialState,
  );

  return (
    <form action={formAction} style={formStyle} noValidate>
      <h2 style={headingStyle}>ユーザー編集</h2>
          ...
          {ROLE_OPTIONS.map((option) => (
            <label key={option.value} style={checkboxLabelStyle}>
              <input
                ...
                name="roles"
                value={option.value}
                defaultChecked={user.roles.includes(option.value as typeof user.roles[number])}
                disabled={isPending}
              />
              {option.label}
            </label>
      ...
      <div style={fieldStyle}>
        <span style={labelStyle}>アカウント状態</span>
